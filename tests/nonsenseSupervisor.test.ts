...
describe("fetchData - nonsense input", () => {
  it("returns empty array or fallback supervisor for nonsense input", async () => {
...
describe("fetchData - nonsense input", () => {
  it("returns empty array or fallback supervisor for semi-nonsense input (nonsense with a keyword)", async () => {
    const res = await fetchData<RecommendationResponse>("/recommend/invoke", {
      method: "POST",
      body: JSON.stringify({
        input: {
